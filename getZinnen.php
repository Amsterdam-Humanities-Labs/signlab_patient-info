...
switch ($action) {
    ...
    case 'saveStatusGvg':
        saveStatusGvg($conn);
        break;
    case 'updateStatusForLabel':
        updateStatusForLabel($conn);
        break;
    ...
    case 'fetchSegments':
        fetchSegments($conn);
        break;
    case 'fetchSegmentsByRow':
        fetchSegmentsByRow($conn);
        break;
    case 'syncSegments':
        syncSegments($conn);
        break;
    case 'fetchSegmentTiers':
        fetchSegmentTiers($conn);
        break;
    case '':
...
function deleteZin($conn){
    ...
    if (!empty($rowId)) {
        ...
        $sql = "DELETE FROM hh_index WHERE id = ?";
...
function updateStatusForLabel($conn){
    $label = $_POST['label'] ?? '';
    $status = $_POST['status'] ?? '';

    logAction($conn, 'updateStatusForLabel', ['label' => $label, 'status' => $status]);

    if (!empty($label) && !empty($status)) {
        $labelJson = json_encode($label);
        $sql = "UPDATE hh_index SET status_video = ? WHERE JSON_CONTAINS(labels, ?)";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            echo json_encode(['success' => false, 'error' => 'Prepare failed: ' . $conn->error]);
            exit();
        }
        $stmt->bind_param("ss", $status, $labelJson);
        if ($stmt->execute()) {
            echo json_encode(['success' => true]);
        } else {
            echo json_encode(['success' => false, 'error' => $stmt->error]);
        ...
        echo json_encode(['success' => false, 'error' => 'Invalid input, received label: ' . $label . ' and status: ' . $status]);
    }
    exit();
}
...
function saveStatusVideo($conn){
    ...
    $deleteVideos = $_POST['deleteVideos'] ?? '';

    logAction($conn, 'saveStatusVideo', ['rowId' => $rowId, 'statusVideo' => $statusVideo, 'deleteVideos' => $deleteVideos]);

    if (!empty($rowId)) {
        // SQL query to update the status in hh_index
        $sql = "UPDATE hh_index SET status_video = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            echo json_encode(['success' => false, 'error' => 'Prepare failed: ' . $conn->error]);
            exit();
        ...
        $videosDeleted = 0;
        // If Opnieuw + deleteVideos flag, set added='DELETE' on associated videos
        if ($statusVideo === 'Opnieuw' && $deleteVideos === '1') {
            $delStmt = $conn->prepare("UPDATE matched_transcriptions SET added = 'DELETE' WHERE m_transcription = ? AND zOg = 'tekst' AND added = '1'");
            $delStmt->bind_param("i", $rowId);
            $delStmt->execute();
            $videosDeleted = $delStmt->affected_rows;
            $delStmt->close();
        }

        echo json_encode(['success' => true, 'videos_deleted' => $videosDeleted]);
    } else {
...
function saveStatusAnnotatie($conn){
    ...
    if (!empty($rowId)) {
        $sql = "UPDATE hh_index SET status_nederlands = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            echo json_encode(['success' => false, 'error' => 'Prepare failed: ' . $conn->error]);
            exit();
...
function saveStatusGlos($conn){
    ...
    if (!empty($rowId)) {
        $sql = "UPDATE hh_index SET status_glos = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            echo json_encode(['success' => false, 'error' => 'Prepare failed: ' . $conn->error]);
            exit();
...
function saveStatusGvg($conn){
    ...
    if (!empty($rowId)) {
        $sql = "UPDATE hh_index SET status_gvg = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            echo json_encode(['success' => false, 'error' => 'Prepare failed: ' . $conn->error]);
            exit();
...
function saveComments($conn){
    ...
    if (!empty($rowId) && !empty($comments)) {
        ...
        $sql = "UPDATE hh_index SET comments = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            echo json_encode(['success' => false, 'error' => 'Prepare failed: ' . $conn->error]);
            exit();
...
/**
 * Fetch Unique Label Values from hh_index.labels JSON column
 */
function fetchLabels($conn) {
    $sql = "SELECT DISTINCT labels FROM hh_index WHERE labels IS NOT NULL AND labels != ''";
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (is_array($decoded)) {
            ...
            // labels might be a plain string
            $label = trim($row['labels']);
            if (!empty($label)) {
                $labelsSet[$label] = true;
            }
        }
...
function duplicateRow($conn) {
    ...
    if (!empty($videoCenter) && !empty($rowId)) {
        ...
        try {
            // Duplicate the row in the hh_index table (without url to avoid UNIQUE constraint)
            $sql = "INSERT INTO hh_index (plain_text, keywords, ngt_text, labels, status_video, status_glos, status_nederlands, status_gvg, comments)
                    SELECT plain_text, keywords, ngt_text, labels, status_video, status_glos, status_nederlands, status_gvg, comments FROM hh_index WHERE id = ?";
            $stmt = $conn->prepare($sql);
            if (!$stmt) {
...
function fetchVideos($conn){
    ...
    if (!empty($rowId)) {
        ...
        if ($includeDeleted === 'true') {
            ...
            $sql = "SELECT *, ID as video_id FROM matched_transcriptions WHERE m_transcription = ? AND zOg = 'tekst' ORDER BY added DESC, ID ASC";
        } else {
            // Only include active videos (added = 1)
            $sql = "SELECT *, ID as video_id FROM matched_transcriptions WHERE m_transcription = ? AND zOg = 'tekst' AND added = 1 ORDER BY ID ASC";
        }
...
function refreshVideoCount($conn, $sentenceId) {
    // Match on the numeric prefix of m_transcription ('8848' and variants like
    // '8848_2' both belong to sentence 8848), consistent with fetchVideos/countZinnen.
    $sentenceId = intval($sentenceId);
    if (empty($sentenceId)) return;
    $stmt = $conn->prepare("UPDATE hh_index SET video_count = (SELECT COUNT(*) FROM matched_transcriptions WHERE CAST(REGEXP_SUBSTR(m_transcription, '^[0-9]+') AS UNSIGNED) = ? AND zOg = 'tekst' AND added = '1') WHERE id = ?");
    $stmt->bind_param("ii", $sentenceId, $sentenceId);
    $stmt->execute();
    $stmt->close();
}
...
function fetchRow($conn) {
    ...
    if (!empty($rowId)) {
        // Fetch the row from the hh_index table
        $sql = "SELECT
                    hh_index.id AS ID,
                    hh_index.plain_text,
                    hh_index.ngt_text,
                    ...
                    hh_index.comments,
                    hh_index.status_video
                FROM hh_index
                LEFT JOIN matched_transcriptions
                    ON hh_index.id = matched_transcriptions.m_transcription
                    AND matched_transcriptions.zOg = 'tekst'
...
function deleteVideo($conn) {
    ...
    if (!empty($filename)) {
        ...
        $idStmt = $conn->prepare("SELECT DISTINCT m_transcription FROM matched_transcriptions WHERE m_file LIKE ? AND zOg = 'tekst' AND added = '1'");
...
function editZin($conn) {
    ...
    if (!empty($rowId) && !empty($zin)) {
        // SQL query to update the plain_text in the database
        $sql = "UPDATE hh_index SET plain_text = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            echo json_encode(['success' => false, 'error' => 'Prepare failed: ' . $conn->error]);
            exit();
        }
        $stmt->bind_param("si", $zin, $rowId);
        if ($stmt->execute()) {
            // Return success
            echo json_encode(['success' => true]);
        } else {
...
function fetchSentences($conn) {
    ...
    $withVideo = $_GET['withVideo'] ?? null;
    $withSegments = $_GET['withSegments'] ?? null;
    ...
    $sql = "SELECT
                ...
                hh_index.comments,
                hh_index.status_video,
                hh_index.status_nederlands AS status_annotatie,
                hh_index.status_glos,
                hh_index.status_gvg,
                COALESCE(hh_index.video_count, 0) AS video_count,
                NULL AS has_segments
            FROM hh_index";
    ...
    $statusFilters = [
        ...
        'status_nederlands' => $status_annotatie,
    ...
    if (!empty($activeStatusFilters)) {
        foreach ($activeStatusFilters as $field => $value) {
            if ($value === 'Niet Klaar') {
                // "Niet Klaar" should match both explicit 'Niet Klaar' and empty/null values
                $whereClauses[] = "(hh_index.$field = 'Niet Klaar' OR hh_index.$field IS NULL OR hh_index.$field = '' OR hh_index.$field = '0')";
            } else {
                $whereClauses[] = "hh_index.$field = ?";
                $types .= 's';
                $params[] = $value;
            }
        }
    ...
    // Label filter using JSON_CONTAINS
    if (!empty($label)) {
        $labelJson = json_encode($label);
        $whereClauses[] = "JSON_CONTAINS(hh_index.labels, ?)";
        $types .= 's';
        $params[] = $labelJson;
    }

    // Search filter on plain_text
    if (!empty($search)) {
        $searchWords = preg_split('/\s+/', trim($search));
        foreach ($searchWords as $word) {
            if (!empty($word)) {
                $whereClauses[] = "LOWER(hh_index.plain_text) LIKE ?";
                $types .= 's';
    ...
    // Search gloss on ngt_text
    if (!empty($searchGloss)) {
        $whereClauses[] = "LOWER(hh_index.ngt_text) LIKE ?";
        $types .= 's';
        $params[] = '%' . strtolower($searchGloss) . '%';
    }

    // Search GVG on ngt_text fields
    if (!empty($searchGvg)) {
        $whereClauses[] = "(LOWER(hh_index.ngt_text) LIKE ? OR LOWER(hh_index.ngt_text2) LIKE ? OR LOWER(hh_index.ngt_text3) LIKE ? OR LOWER(hh_index.ngt_text4) LIKE ?)";
        $types .= 'ssss';
        $searchGvgLower = '%' . strtolower($searchGvg) . '%';
        $params[] = $searchGvgLower;
        $params[] = $searchGvgLower;
        $params[] = $searchGvgLower;
        $params[] = $searchGvgLower;
    }

    // Filter items without any video (uses maintained video_count column)
    if (!empty($withoutVideo) && $withoutVideo === '1') {
        $whereClauses[] = "COALESCE(hh_index.video_count, 0) = 0";
    }

    // Filter items with video
    if (!empty($withVideo) && $withVideo === '1') {
        $whereClauses[] = "COALESCE(hh_index.video_count, 0) > 0";
    }

    // Filter items with segments
    if (!empty($withSegments) && $withSegments === '1') {
        $whereClauses[] = "EXISTS (
            SELECT 1 FROM matched_transcriptions mt_seg
            INNER JOIN hh_segments hs ON hs.base_filename = REPLACE(mt_seg.m_file, '.wav', '')
            WHERE mt_seg.m_transcription = hh_index.id
            AND mt_seg.zOg = 'tekst'
            AND mt_seg.added = '1'
        )";
    } elseif (!empty($withSegments) && $withSegments === '0') {
        $whereClauses[] = "NOT EXISTS (
            SELECT 1 FROM matched_transcriptions mt_seg2
            INNER JOIN hh_segments hs2 ON hs2.base_filename = REPLACE(mt_seg2.m_file, '.wav', '')
            WHERE mt_seg2.m_transcription = hh_index.id
            AND mt_seg2.zOg = 'tekst'
            AND mt_seg2.added = '1'
        )";
    }
    ...
    $countSql = "SELECT COUNT(DISTINCT hh_index.id) as total FROM hh_index";
    if (!empty($whereClauses)) {
    ...
    // Video count
    $videoCountSql = "SELECT COALESCE(SUM(video_count), 0) as total_videos FROM hh_index";
    if (!empty($whereClauses)) {
    ...
    // Rows with videos first, rows without videos last
    $sql .= " ORDER BY (COALESCE(hh_index.video_count, 0) > 0) DESC, hh_index.id DESC";
    ...
    if (!empty($rowIDs)) {
        ...
        $video_sql = "SELECT mt.*,
                        (SELECT COUNT(*) FROM hh_segments hs
                         WHERE hs.base_filename = REPLACE(mt.m_file, '.wav', '')) AS segment_count
                      FROM matched_transcriptions mt
                      WHERE mt.zOg='tekst' AND mt.m_transcription IN ($placeholders) AND mt.added = 1
                      ORDER BY mt.ID ASC";
        ...
        $video_types = str_repeat('i', count($rowIDs));
        $video_stmt->bind_param($video_types, ...$rowIDs);
...
function downloadMP4($conn) {
    ...
    foreach ($sanitizedFilenames as $index => $sanitizedFile) {
        ...
        $eafPath = __DIR__ . '/eaf/' . preg_replace('/\.(mp4)$/i', '.eaf', $sanitizedFile);
...
function logAction($conn, $action, $parameters = []) {
    ...
    // Get username from sessionObject cookie
    $username = null;
    if (isset($_COOKIE['sessionObject'])) {
        $sessionData = json_decode($_COOKIE['sessionObject'], true);
    ...
    // Prepare the SQL statement
    $sql = "INSERT INTO hh_logs (action, parameters, ip_address, username) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
    ...
    // Bind parameters
    $stmt->bind_param("ssss", $action, $parametersJson, $ipAddress, $username);
...
function fetchLogs($conn) {
    ...
    $countSQL = "SELECT COUNT(*) as total FROM hh_logs $whereSQL";
    $countStmt = $conn->prepare($countSQL);
    if (!$countStmt) {
        echo json_encode(['success' => false, 'error' => 'Prepare failed: ' . $conn->error]);
        exit();
    ...
    $sql = "SELECT action, parameters, ip_address, datetime FROM hh_logs $whereSQL ORDER BY datetime DESC LIMIT ? OFFSET ?";
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
        echo json_encode(['success' => false, 'error' => 'Prepare failed: ' . $conn->error]);
        exit();
...
function generateJSONFeed($conn) {
    ...
    $srtBaseURL = 'https://signcollect.nl/hh/eaf/'; // URL to SRT files
    $domainURL = 'https://signcollect.nl/'; // Base domain URL

    // Fetch data: matched_transcriptions joined with sentences
    $sql = "SELECT mt.a_file, mt.b_file, mt.m_file, mt.r_file, mt.l_file, mt.zOg, mt.post_processed, s.plain_text, s.labels, s.id
            FROM matched_transcriptions mt
            JOIN hh_index s ON mt.m_transcription = s.id
            WHERE mt.zOg = 'tekst' AND mt.added = 1 AND mt.post_processed = 1";
    ...
    $zinnen = [];
    while ($row = $result->fetch_assoc()) {
        // Use plain_text directly
        $plainText = $row['plain_text'] ?? '';
        if (empty($plainText)) {
            continue;
        }

        // Get first line and capitalize
        $zinSentence = ucfirst(strtok($plainText, "\n"));
        ...
        $zinnen[] = [
            'zinId' => $unique_id,
            'zin' => $zinSentence,
            'thema' => $row['labels'] ?: 'Uncategorized',
            'videos' => [
                'a_file' => $videoURLs[0],
                'b_file' => $videoURLs[1],
                'm_file' => $videoURLs[2],
...
function downloadZIP($conn)
{
    ...
    $sql = "SELECT a_file, b_file, m_file, l_file, r_file FROM matched_transcriptions WHERE m_file = ? AND zOg = 'tekst'";
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
        echo json_encode(['success' => false, 'error' => 'Prepare failed: ' . $conn->error]);
        exit();
    ...
    //we are going to forloop with wildcard $filename with .srt extension in eaf
...
function verifySubtitles($conn) {
    ...
    $stmt = $conn->prepare("SELECT parameters FROM hh_logs WHERE action = 'saveSubtitles' AND parameters LIKE ? ORDER BY datetime DESC LIMIT 1");
...
//count rows in hh_index based on various status fields
//also count .eaf files without backup in /web/hh/eaf
...
function countZinnen($conn)
{
    ...
    if (file_exists($cacheFile) && (time() - filemtime($cacheFile)) < $cacheDuration) {
        ...
        if ($cachedData && (isset($cachedData['total_items']) || isset($cachedData['total_sentences']))) {
            header('Content-Type: application/json; charset=utf-8');
            echo $cached;
            exit();
        }
    ...
    // Aggregate counts: single pass over hh_index. Uses the maintained
    // video_count column instead of correlated subqueries against
    // matched_transcriptions (m_transcription is TEXT vs hh_index.id INT,
    // which makes those joins unindexable and extremely slow).
    $sql = "SELECT
...
SUM(CASE WHEN COALESCE(status_video, '0') = 'Klaar'
       AND COALESCE(status_nederlands, '0') = 'Klaar'
...
SUM(CASE WHEN COALESCE(status_nederlands, '0') = 'Klaar' THEN 1 ELSE 0 END) AS count_status_annotatie_klaar,
...
SUM(CASE WHEN COALESCE(status_nederlands, '0') <> 'Klaar' THEN 1 ELSE 0 END) AS count_status_annotatie_not_klaar,
SUM(CASE WHEN COALESCE(status_glos, '0') <> 'Klaar' THEN 1 ELSE 0 END) AS count_status_glos_not_klaar,
SUM(CASE WHEN COALESCE(status_nederlands, '0') = 'Controle nodig' OR COALESCE(status_gvg, '0') = 'Check nodig' THEN 1 ELSE 0 END) AS count_josje_check,
SUM(CASE WHEN COALESCE(video_count, 0) = 0 THEN 1 ELSE 0 END) AS sentences_without_video,
SUM(CASE WHEN COALESCE(video_count, 0) > 0 THEN 1 ELSE 0 END) AS sentences_with_video,
(SELECT COUNT(*) FROM matched_transcriptions WHERE zOg = 'tekst' AND added = 1) AS total_videos_in_mt
    FROM hh_index";
    $result = $conn->query($sql);
    $counts = ($result && $row = $result->fetch_assoc()) ? $row : [];

    // Count videos for each status category (only active videos: added=1, zOg='tekst').
    // One pass: aggregate matched_transcriptions per sentence first (derived table,
    // hash-joined to hh_index) instead of ten unindexable correlated subqueries.
    $videoSql = "SELECT
        (SELECT COUNT(*) FROM matched_transcriptions WHERE added = 1 AND zOg = 'tekst') AS total_videos,
        COALESCE(SUM(CASE WHEN COALESCE(s.status_video, '0') = 'Klaar'
                          AND COALESCE(s.status_nederlands, '0') = 'Klaar'
                          AND COALESCE(s.status_glos, '0') = 'Klaar'
                          AND COALESCE(s.status_gvg, '0') = 'Klaar' THEN v.c ELSE 0 END), 0) AS video_ready_sentences,
        COALESCE(SUM(CASE WHEN COALESCE(s.status_video, '0') = 'Klaar' THEN v.c ELSE 0 END), 0) AS video_count_status_video_klaar,
        COALESCE(SUM(CASE WHEN COALESCE(s.status_video, '0') <> 'Klaar' THEN v.c ELSE 0 END), 0) AS video_count_status_video_not_klaar,
        COALESCE(SUM(CASE WHEN COALESCE(s.status_nederlands, '0') = 'Klaar' THEN v.c ELSE 0 END), 0) AS video_count_status_annotatie_klaar,
        COALESCE(SUM(CASE WHEN COALESCE(s.status_nederlands, '0') <> 'Klaar' THEN v.c ELSE 0 END), 0) AS video_count_status_annotatie_not_klaar,
        COALESCE(SUM(CASE WHEN COALESCE(s.status_glos, '0') = 'Klaar' THEN v.c ELSE 0 END), 0) AS video_count_status_glos_klaar,
        COALESCE(SUM(CASE WHEN COALESCE(s.status_glos, '0') <> 'Klaar' THEN v.c ELSE 0 END), 0) AS video_count_status_glos_not_klaar,
        COALESCE(SUM(CASE WHEN COALESCE(s.status_gvg, '0') = 'Klaar' THEN v.c ELSE 0 END), 0) AS video_count_status_gvg_klaar,
        COALESCE(SUM(CASE WHEN COALESCE(s.status_gvg, '0') <> 'Klaar' THEN v.c ELSE 0 END), 0) AS video_count_status_gvg_not_klaar,
        COALESCE(SUM(CASE WHEN COALESCE(s.status_nederlands, '0') = 'Controle nodig'
                          OR COALESCE(s.status_gvg, '0') = 'Check nodig' THEN v.c ELSE 0 END), 0) AS video_count_josje_check
        FROM hh_index s
        INNER JOIN (SELECT CAST(m_transcription AS UNSIGNED) AS mid, COUNT(*) AS c
                    FROM matched_transcriptions
                    WHERE added = 1 AND zOg = 'tekst' GROUP BY mid) v
            ON v.mid = s.id";
    ...
    if ($eafDoc->asXML($eafPath)) {
        ...
        if ($row && !empty($row['m_transcription'])) {
            ...
            $updateStmt = $conn->prepare(
                "UPDATE hh_index SET ngt_text=?, ngt_text2=?, ngt_text3=? WHERE id=?"
            );
...
function findgvg($conn){
    ...
    $likegvg = "%" . $gvg . "%";
    $sql = "SELECT * FROM hh_index WHERE ngt_text LIKE ?";
    $stmt = $conn->prepare($sql);
    if(!$stmt){
        echo json_encode(['success' => false, 'error' => 'Prepare failed: ' . $conn->error]);
        exit();
...
function fetchZinArrayByFilename($conn) {
    ...
    // First, find the hh_index.id (m_transcription) from matched_transcriptions
    $sql_mt = "SELECT m_transcription FROM matched_transcriptions WHERE m_file = ? AND zOg = 'tekst' LIMIT 1";
    ...
    // Now, fetch the plain_text from the hh_index table using the ID
    $sql_s = "SELECT plain_text, id as sentenceRowId, status_video, status_nederlands AS status_annotatie, status_glos, status_gvg FROM hh_index WHERE id = ? LIMIT 1";
    $stmt_s = $conn->prepare($sql_s);
    if (!$stmt_s) {
        echo json_encode(['success' => false, 'error' => 'Prepare failed (hh_index): ' . $conn->error]);
        exit();
    }
    $stmt_s->bind_param("i", $sentenceId);
    if (!$stmt_s->execute()) {
        echo json_encode(['success' => false, 'error' => 'Execute failed (hh_index): ' . $stmt_s->error]);
        exit();
    }
...
function getOverviewStats($conn) {
    // Total actions
    $totalQuery = "SELECT COUNT(*) as total FROM hh_logs";
    $totalResult = $conn->query($totalQuery);
    $totalActions = $totalResult->fetch_assoc()['total'];
    
    // Unique users (by username, fallback to ip_address)
    $usersQuery = "SELECT COUNT(DISTINCT COALESCE(username, ip_address)) as unique_users FROM hh_logs";
    $usersResult = $conn->query($usersQuery);
    $uniqueUsers = $usersResult->fetch_assoc()['unique_users'];
    
    // Today's actions
    $todayQuery = "SELECT COUNT(*) as today FROM hh_logs WHERE DATE(datetime) = CURDATE()";
    $todayResult = $conn->query($todayQuery);
    $todayActions = $todayResult->fetch_assoc()['today'];
    
    // Yesterday's actions for comparison
    $yesterdayQuery = "SELECT COUNT(*) as yesterday FROM hh_logs WHERE DATE(datetime) = DATE_SUB(CURDATE(), INTERVAL 1 DAY)";
    ...
    // Top action
    $topActionQuery = "SELECT action, COUNT(*) as count FROM hh_logs GROUP BY action ORDER BY count DESC LIMIT 1";
...
function getTopUsers($conn) {
    $query = "
        SELECT
            COALESCE(username, ip_address) as user_identifier,
            username,
            ip_address,
            ...
            (
                SELECT action
                FROM hh_logs sl2
                WHERE COALESCE(sl2.username, sl2.ip_address) = COALESCE(sl1.username, sl1.ip_address)
                GROUP BY action
                ORDER BY COUNT(*) DESC
                LIMIT 1
            ) as topAction
        FROM hh_logs sl1
        GROUP BY user_identifier, username, ip_address
...
/**
 * Fetch segments for a given base_filename
 * GET ?action=fetchSegments&base_filename=M20250610_9217
 */
function fetchSegments($conn) {
    $baseFilename = $_GET['base_filename'] ?? '';

    if (empty($baseFilename)) {
        echo json_encode(['success' => false, 'error' => 'base_filename parameter required']);
        return;
    }

    $stmt = $conn->prepare("SELECT * FROM hh_segments WHERE base_filename = ? ORDER BY segment_number ASC");
...
/**
 * Fetch all segments for all videos of an hh_index item
 * GET ?action=fetchSegmentsByRow&rowId=456
 */
function fetchSegmentsByRow($conn) {
    $rowId = $_GET['rowId'] ?? '';

    if (empty($rowId)) {
        echo json_encode(['success' => false, 'error' => 'rowId parameter required']);
        return;
    }
    ...
    $sql = "SELECT hs.*
            ...
            INNER JOIN matched_transcriptions mt
              ON REPLACE(mt.m_file, '.wav', '') = hs.base_filename
            WHERE mt.m_transcription = ?
              AND mt.zOg = 'tekst'
              AND mt.added = '1'
            ORDER BY hs.base_filename, hs.segment_number";
    ...
    $totalSegments = array_sum(array_map('count', $grouped));

    echo json_encode([
        'success' => true,
        'segments_by_video' => $grouped,
        'total_videos' => count($grouped),
        'total_segments' => $totalSegments
    ]);
}

/**
 * Return current segment counts from DB
 * GET/POST ?action=syncSegments
 * (DB is kept up to date by segment_api.php on upload — no filesystem scan needed)
 */
function syncSegments($conn) {
    $result = $conn->query("SELECT COUNT(*) as cnt, COUNT(DISTINCT base_filename) as bases FROM hh_segments");
    $counts = $result->fetch_assoc();

    echo json_encode([
        'success' => true,
        'message' => 'Segment counts from DB',
        'total_segments' => (int)$counts['cnt'],
        'total_bases' => (int)$counts['bases']
    ]);
}

/**
 * Fetch all segments for a base_filename with tier annotation counts.
 * GET ?action=fetchSegmentTiers&base_filename=M20251223_9344
 * Counts SRT entries per tier by reading the SRT files in /eaf/.
 */
function fetchSegmentTiers($conn) {
    $baseFilename = $_GET['base_filename'] ?? '';
    if (empty($baseFilename)) {
        echo json_encode(['success' => false, 'error' => 'base_filename parameter required']);
        return;
    ...
    $eafDir = __DIR__ . '/eaf/';
    $tierNames = ['Nederlands', 'Gebaar-voor-gebaar', 'Signbank_ID_glossen'];
    $segments = [];

    while ($row = $result->fetch_assoc()) {
        $segBase = $baseFilename . '_' . $row['segment_number'];
        $tiers = [];
        foreach ($tierNames as $tier) {
            $srtPath = $eafDir . $segBase . '_' . $tier . '.srt';
            $count = 0;
            if (file_exists($srtPath)) {
                $content = file_get_contents($srtPath);
                // Count SRT entries: each entry starts with a sequence number on its own line
